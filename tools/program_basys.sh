...
if [ ! -f "$BIN_FILE" ]; then
    echo "Blad: Nie znaleziono pliku QSPI: $BIN_FILE"
    echo "Uruchom najpierw generowanie bitstreamu w katalogu projektu, np.:"
    echo "  cd $1 && ./tools/generate_bitstream.sh"
    exit 1
fi

if [ "$BITSTREAM" -nt "$BIN_FILE" ]; then
    echo "Blad: Plik QSPI jest starszy niz bitstream: $BIN_FILE"
    echo "Wygeneruj ponownie bitstream, zeby odswiezyc takze plik QSPI."
    exit 1
fi
